...
db = pymysql.connect(host="127.0.0.1", user="root", passwd="", db="testdb", port=3306 )
cursor = db.cursor()
...
@app.route('/filres', methods=['GET'])
def api_id():
    if 'id' in request.args:
        ids = int(request.args['id'])
    else:
...
def checkmail(email):
    query = "select * from regdata where email=%s"
    try:
...
def login():
    ...
    if details['email'] != '' and details['password'] != '':
        try:
            ...
            if not checkmail(email):
                ...
                if passwd[1] == pwd:
                    ...
                    print(printval)
                    """
                    token = jwt.encode({"user": em[1]}, app.config["SECRET_KEY"], algorithm='HS256')

                    """
                    For Backend using Form
                    ...
                    return json.dumps({'token': token.decode('UTF-8'), 'success': True, 'message': 'User Login Successfull', 'email': em, 'name': nm, 'contact': num}), 200, {'ContentType': 'application/json'}
                else:
        ...
        return json.dumps({'success': False, 'Message': 'All Credentials Not Set'}), 401, {
